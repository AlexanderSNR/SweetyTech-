...
//----------------------------------------------------------------------------------------------------------
    ...
    {
        $stock = "SELECT * FROM tbl_insumo WHERE cantidad<=StockMinimo";
...
//----------------------------------------------------------------------------------------------------------
    ...
    {
        $consulta = "SELECT Nombre_Insumo,cantidad FROM tbl_insumo WHERE cantidad<=StockMinimo";
    ...
    public function insertar(salida $salida)
    {
        ...
        try {
            ...
            foreach ($insumosTmp->fetchAll(PDO::FETCH_OBJ) as $date) {
                ...
                $selecCantidad="SELECT * from tbl_insumo  where Codigo_Insumo= $date->Codigo_Insumo";
                $cantidad=$this->conexion->prepare($selecCantidad);
                $cantidad->execute();
                $cantidadO = $cantidad->fetch(PDO::FETCH_OBJ);
                $totalCantidad= $cantidadO->cantidad - $date->cantidad;
                $actualizarCantidad = "UPDATE `tbl_insumo` SET cantidad = ? WHERE Codigo_Insumo =?";
                $this->conexion->prepare($actualizarCantidad)->execute(array(
                    $totalCantidad,
                    (int) $date->Codigo_Insumo,
                ));
            }
            /*
            $v1=$this->Cantidad;
            consulta2="UPDATE tbl_insumo set cantidad= $v1-cantidad where Codigo_insumo=Codigo_insumo";
